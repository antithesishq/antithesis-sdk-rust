...
macro_rules! assert_helper {
    ...
    (condition = $condition:expr, $message:literal, $(details = $details:expr)?, $assert_type:path, $display_type:literal, must_hit = $must_hit:literal) => {{
        ...
        static sometimes_CATALOG_ITEM: AssertionCatalogInfo = AssertionCatalogInfo {
...
macro_rules! assert_helper {
    (condition = $condition:expr, $message:literal, $(details = $details:expr)?, $assert_type:path, $display_type:literal, must_hit = $must_hit:literal) => {{
        ...
        // any side effects of these expressions will sometimes be
...
/// called at least once. The corresponding test property will be viewable in the ``Antithesis SDK: sometimes`` group of your triage report.
///
/// # Example
///
/// ```
...
/// This test property will be viewable in the ``Antithesis SDK: sometimes`` group of your triage report.
///
/// # Example
///
/// ```
/// use serde_json::{json};
/// use antithesis_sdk::{assert_sometimes_or_unreachable, random};
///
/// const MAX_ALLOWED: u64 = 100;
/// let actual = random::get_random() % 100u64;
/// let details = json!({"max_allowed": MAX_ALLOWED, "actual": actual});
/// antithesis_sdk::assert_sometimes_or_unreachable!(actual < MAX_ALLOWED, "Value in range", &details);
/// ```
#[macro_export]
macro_rules! assert_sometimes_or_unreachable {
    ($condition:expr, $message:literal$(, $details:expr)?) => {
        $crate::assert_helper!(
            condition = $condition,
            $message,
            $(details = $details)?,
            $crate::assert::AssertType::sometimes,
            "sometimesOrUnreachable",
            must_hit = false
        )
    };
    ($($rest:tt)*) => {
        ::std::compile_error!(
r#"Invalid syntax when calling macro `assert_sometimes_or_unreachable`.
Example usage:
    `assert_sometimes_or_unreachable!(condition_expr, "assertion message (static literal)", &details_json_value_expr)`
"#
        );
    };
}
...
/// `assert_sometimes_some({a: x, b: y, ...})` is similar to `assert_sometimes(x || y || ...)`, except:
...
#[macro_export]
macro_rules! assert_sometimes_some {
    ({$($($name:ident: $cond:expr),+ $(,)?)?}, $message:literal$(, $details:expr)?) => {
        $crate::boolean_guidance_helper!($crate::assert_sometimes, false, {$($($name: $cond),+)?}, $message$(, $details)?);
    };
    ($($rest:tt)*) => {
        ::std::compile_error!(
r#"Invalid syntax when calling macro `assert_sometimes_some`.
Example usage:
    `assert_sometimes_some!({field1: cond1, field2: cond2, ...}, "assertion message (static literal)", &details_json_value_expr)`
"#
        );
    };
}
